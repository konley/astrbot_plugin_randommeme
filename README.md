...
一个 AstrBot 插件：用户发送关键词，从对应「组别」目录中**随机且不重复**地抽取一张图片发送。支持多组别、别名、需唤醒开关、WebUI 管理，以及**聊天精简指令**（加图 / 新建组别 / 改别名）。
...
- **关键词抽图**：默认模糊匹配（文本包含关键词即触发，最长优先）；可在配置中改为精确匹配。
- **多组别独立目录**：每个组别对应一个图片目录，互不干扰。
- **别名**：每个组别可设置多个别名。
- **需唤醒开关**：可指定某些组别在 @机器人 或带前缀的情况下才触发。
- **随机 + 不重复**：一整轮抽取完之后才洗牌，全局共享抽取池。
- **支持动图**：JPG / PNG / WEBP / BMP / GIF 全开。
- **WebUI 管理**：内置 Plugin Page，含组别 CRUD、批量上传、批量删除、重置抽取序列、统计。
- **聊天精简指令**：`表情 列表 / 加 / 新 / 别名 / 帮助`，可在群里灌图、建组、改别名。
- **仅管理员可管理**：配置开关；开启后聊天写操作仅 AstrBot 管理员可用，**不影响所有人出图**；WebUI 始终可用。
- **数据隔离**：所有持久化数据落在 `data/plugin_data/astrbot_plugin_randommeme/`。
...
```text
表情 新 摸鱼 moyu          # 建组 + 初始别名
表情 加 摸鱼               # 同条带图，或回复一张图，或发完 30 秒内再发图
摸鱼                       # 出图
表情 别名 摸鱼 moyu 摸一下  # 覆盖别名
表情 列表                  # 看库
表情 帮助                  # 查看指令
```

> 指令属于 AstrBot **标准指令**，需要 `@机器人` 或全局唤醒前缀（如 `/`）才能触发；具体以你的 Bot 唤醒配置为准。  
> 写操作（加 / 新 / 别名）在开启「仅管理员可管理」时仅 AstrBot 管理员可用。

### 方式 B：WebUI

1. WebUI → 随机表情包 → 组别管理 → 新建组别  
2. 图片管理 → 拖拽上传  
3. 群里发送组别名 / 别名出图
...
| 发送 `  摸鱼  ` | ✅ 自动 strip |
| 发送 `MOYU` | ✅ 命中别名（大小写不敏感） |
| 发送 `摸鱼一下` 但仅定义了 `摸鱼` 和别名 `moyu`，且 `exact_match=true` | ❌ 不命中 |
| 发送 `看doro` 且组别为 `doro`，`exact_match=false`（默认） | ✅ 模糊命中 |
...
- 手动重置抽取序列：
  ...
  - WebUI → 图片管理 → **重置该组抽取序列**
  - 兼容指令：`随机meme重置 <组别>` 或 `随机meme重置`（清空全部）；日常更推荐 WebUI
...
|------|------|------|
| `表情 帮助` | 查看指令 | 所有人 |
| `表情 列表` | 列出组别、图数、别名 | 所有人 |
| `表情 加 <组别>` | 聊天加图：同条附图 / 回复带图 / 发指令后 30 秒内再发图 | 管理（可关） |
| `表情 新 <名称> [别名…]` | 新建组别，可写初始别名 | 管理（可关） |
| `表情 别名 <组别> <别名…>` | **覆盖**该组别全部别名 | 管理（可关） |
| 直接发组别名/别名 | 随机抽一张图 | 所有人（受抽图配置约束） |
...
| 指令 | 说明 |
|------|------|
| `随机meme` / `随机表情` / `包帮助` | 同「表情 帮助」 |
| `随机meme列表` / `meme列表` | 同「表情 列表」 |
| `随机meme详情 <组别>` | 单组详情 |
| `随机meme重置 [组别]` | 重置抽取序列（受「仅管理员可管理」约束） |
| `随机meme禁用` / `随机meme启用` | 启停组别（受「仅管理员可管理」约束） |

> 标准指令需唤醒（`@` 或全局 `wake_prefix`）。抽图关键词走消息钩子，受插件配置 `need_prefix` / 组别「需唤醒」控制。
...
| Key | 类型 | 默认 | 说明 |
|-----|------|------|------|
| `need_prefix` | bool | `true` | 是否需要 @机器人 / 唤醒前缀才能**抽图**。关闭后可被任何文本直接触发（**小心与 LLM 误命中**）。 |
| `extra_prefix` | string | 空 | 可选的额外抽图前缀，例如 `/`；为空则不启用。 |
| `gif_support` | bool | `true` | 是否支持 GIF。关闭后 `.gif` 上传会被忽略。 |
| `exact_match` | bool | `false` | 精确匹配关键词。关则为模糊（包含即触发，最长优先）。 |
| `reply_mode` | bool | `false` | 出图时引用触发消息。 |
| `cooldown_seconds` | int | `0` | 同会话抽图冷却秒数；`0` 不限。 |
| `admin_only_manage` | bool | `true` | **仅管理员可聊天管理**（加/新/别名及兼容的重置启停）。关则所有人可管理。**不影响出图**；WebUI 始终可用。管理员指 AstrBot 控制台配置的管理员。 |
